...
(module+ add-book/article
  (require (submod "classes.rkt" TMP-add-profileDesc))
  (provide (contract-out
            [add-book/article
             (->i ([obj (and/c (is-a?/c TEI<%>)
                               (λ (obj)
                                 (not (send obj get-book/article))))]
                   [which (or/c 'book 'article)])
                  [_ (obj which)
                     (and/c (is-a?/c TEI<%>)
                            (λ (updated)
                              (eq? which (send updated get-book/article))))])]))
  
  (define (make-profileDesc-xexpr book/article-target)
    `(profileDesc
      (textClass
       (catRef ([scheme "https://schema.digitalricoeur.org/taxonomy/type"]
                [target ,book/article-target]))
       (keywords ([scheme
                   "https://schema.digitalricoeur.org/tools/tei-guess-paragraphs"])
                 (term "todo")))))

  (define book-profileDesc
    (tag->element
     (make-profileDesc-xexpr 
      "https://schema.digitalricoeur.org/taxonomy/type#book")))

  (define article-profileDesc
    (tag->element
     (make-profileDesc-xexpr 
      "https://schema.digitalricoeur.org/taxonomy/type#article")))


  (define (add-book/article obj which)
    (send obj TMP-add-profileDesc (case which
                                    [(book) book-profileDesc]
                                    [else article-profileDesc]))))
