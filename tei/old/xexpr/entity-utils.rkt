...
(define-syntax (define-symbolic-entity-utils stx)
  ...
  (syntax-parse stx
    [(_ clause:entity-clause ...+)
     (with-syntax ([pairs:symbol+char 
                    (datum->syntax stx 'pairs:symbol+char)]
                   [entity-symbol/c 
                    (datum->syntax stx 'entity-symbol/c)]
                   [entity-char/c 
                    (datum->syntax stx 'entity-char/c)]
                   [entity-char->symbol 
                    (datum->syntax stx 'entity-char->symbol)]
                   [entity-symbol->char
                    (datum->syntax stx 'entity-symbol->char)])
       #`(begin
           (module+ main
             (define pairs:symbol+char
               `((clause.name . clause.ch) ...)))
           (define entity-symbol/c
             (or/c 'clause.name ...))
           (define entity-char/c
             (or/c clause.ch ...))
